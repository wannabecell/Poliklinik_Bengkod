            ...
            <div class="card bg-base-100 shadow-md rounded-2xl border border-slate-200 sticky top-24">
                <div class="card-body p-6">
                    ...
                    <div class="bg-amber-50 p-4 rounded-xl border border-amber-100 mt-2">
                        ...
                        <p class="text-sm font-medium text-amber-900 leading-relaxed">{{ $daftarPoli->keluhan }}</p>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="card-body p-7">
                    ...
                    <form action="{{ route('periksa.store') }}" method="POST">
                        ...
                        <div class="form-control mb-5">
                            ...
                            <input type="datetime-local" name="tgl_periksa" value="{{ old('tgl_periksa', \Carbon\Carbon::now()->format('Y-m-d\TH:i')) }}"
                        ...
                        <div class="bg-slate-50 border border-slate-200 rounded-xl p-4 mb-8 h-64 overflow-y-auto">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    ...
                                    <label class="cursor-pointer label justify-start gap-4 p-3 rounded-lg border border-transparent hover:border-slate-300 hover:bg-white transition bg-white shadow-sm">
                                        <input type="checkbox" name="obats[]" value="{{ $obat->id }}" 
                                            ...
                                            {{ in_array($obat->id, old('obats', [])) ? 'checked' : '' }} />
                                        <div class="flex-1">
                            ...
                            @else
                                <p class="text-sm text-slate-500 italic text-center my-4">Database obat masih kosong.</p>
                            @endif
                        </div>

                        {{-- Info Biaya --}}
                        <div class="bg-indigo-50 text-indigo-900 border border-indigo-100 rounded-xl p-4 flex flex-col md:flex-row items-center justify-between gap-4 mb-6">
                            <div class="flex items-start gap-3">
                                <i class="fas fa-info-circle text-indigo-500 mt-1"></i>
                                <div>
                                    <p class="text-sm font-bold">Informasi Biaya</p>
                                    <p class="text-xs mt-1 leading-relaxed opacity-80">
                                        Biaya Jasa Dokter/Poli telah ditetapkan sebesar <b>Rp 150.000</b>. Jika terdapat peresepan obat, sistem akan mengotomatiskan penjumlahannya pada tagihan akhir pasien.
                                    </p>
                                </div>
                            </div>
                        </div>
